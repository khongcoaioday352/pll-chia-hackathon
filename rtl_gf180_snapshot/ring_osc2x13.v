...
module delay_stage(in, trim, out);
    parameter real stage_delay_base = 0.18;
    parameter real stage_delay_per_trim = 0.06;
    input in;
    input [1:0] trim;
    output out;
...
`ifdef DELAY_PER_STAGE
    // Guard delay (180ps) prevents zero-delay combinational loops.
    // Extra 60ps per trim bit models the longer path through extra gates.
    wire base_del, trim0_del, trim1_del;
    buf #stage_delay_base (base_del, out_pre);
    buf #stage_delay_per_trim (trim0_del, base_del);
    buf #stage_delay_per_trim (trim1_del, trim0_del);
    assign out = (trim[0] || trim[1])
        ? (trim[0] && trim[1] ? trim1_del : trim0_del)
        : base_del;
`else
    assign out = out_pre;
`endif
...
module ring_osc2x13(reset, trim, clockp);
    parameter real delay_base = 1.168;
    parameter real delay_per_bit = 0.012;
    parameter real stage_delay_base = 0.18;
    parameter real stage_delay_per_trim = 0.06;
    input reset;
    input [25:0] trim;
...
`ifdef FUNCTIONAL	// i.e., behavioral model below
    ...
    always @(*) begin
	bcount = 0;
	for (i = 0; i < 26; i = i + 1) bcount = bcount + trim[i];
    end
...
`else 			// !FUNCTIONAL;  i.e., gate level netlist below
	...
	for (i = 0; i < 12; i = i + 1) begin : dstage
	    delay_stage #(
		.stage_delay_base(stage_delay_base),
		.stage_delay_per_trim(stage_delay_per_trim)
	    ) id (
		.in(d[i]),
