...
The simulator adapter has been smoke-tested on the public Efabless RTL: the
four injected mutants fail corresponding fixed tests while the original passes.
See `SMOKE_RESULTS.json`. The new authored-test adapter has also passed a
single test on the GF180 snapshot in `rtl_gf180_snapshot/` and that test
distinguished an enable mutant; see `GF180_SMOKE_RESULTS.json`. The example
was handwritten, not generated by an agent. The **CHIA AI loop has not yet run end-to-end**.
Do not report smoke-test outputs as competition results. Confirm the snapshot
against the current lab-server RTL and run the full loop before drafting any
numerical claim.

`MODEL_SMOKE_RESULTS.json` records a separate direct OpenCode CLI call: the
model proposed a trim test, which passed the original and detected one of two
checked mutants. Another mutant escaped because its measured edge counts
still differed by one. This call bypassed Ray and CHIA, so it only checks the
model-to-simulator path and illustrates why stronger assertions are needed.
The stronger competition protocol is in `EXPERIMENT_PLAN.md`. The updated
loop accepts agent-authored step sequences and assertions; their quality has
not been measured yet. Do not claim an improvement without a real full run.
